...
function ink_go(ink, ink_type) {
	...
	if (ink_type == "guest_put") { //客人墨水注入-航海见识探索的嘛
		ink_inject(ink, -1, SELECT_INK); //直接注入收工
		/* 聚集焦点 */
	...
	{
		return ink_inject(ink, -1, UNSELECT_INK); //直接注入收工
	};
	...
	/* 好了，送入墨水 */
	ink_inject(ink, curr_pos, UNSELECT_INK);
	/* 传送拨动墨水注射动画 */

}

//当前光标写入文字
function ink_inject(ink, curr_pos, need_select) {
	var editor = ink_get_editor();
